...
public:
  ...
  io_service(int entries = 64, uint32_t flags = 0, uint32_t wq_fd = 0) {
    ...
    io_uring_queue_init_params(entries, &ring_, &p) |
...
#undef TEST_IORING_FEATURE
  ...
  ~io_service() noexcept { io_uring_queue_exit(&ring_); }
...
public:
  ...
  sqe_awaitable statx(int dfd, const char *path, int flags, unsigned mask,
                      struct statx *statxbuf, uint8_t iflags = 0) noexcept {
    auto *sqe = io_uring_get_sqe_safe();
  ...
  sqe_awaitable splice(int fd_in, loff_t off_in, int fd_out, loff_t off_out,
                       size_t nbytes, unsigned flags, uint8_t iflags = 0) {
    auto *sqe = io_uring_get_sqe_safe();
  ...
  sqe_awaitable symlinkat(const char *target, int newdirfd,
                          const char *linkpath, uint8_t iflags = 0) {
    auto *sqe = io_uring_get_sqe_safe();
  ...
  sqe_awaitable linkat(int olddirfd, const char *oldpath, int newdirfd,
                       const char *newpath, int flags, uint8_t iflags = 0) {
    auto *sqe = io_uring_get_sqe_safe();
...
public:
   ...
   * @param ring_ pointer to inited io_uring struct
...
public:
  ...
  void register_files(const int *files, unsigned int nr_files) {
    io_uring_register_files(&ring_, files, nr_files) |
  ...
  void register_files_update(unsigned off, int *files, unsigned nr_files) {
    io_uring_register_files_update(&ring_, off, files, nr_files) |
        panic_on_err("io_uring_register_files", false);
  }
...
public:
   ...
   */
  template <unsigned int N> void register_buffers(iovec (&&ioves)[N]) {
    register_buffers(&ioves[0], N);
  }
  void register_buffers(const struct iovec *iovecs, unsigned nr_iovecs) {
    io_uring_register_buffers(&ring_, iovecs, nr_iovecs) |
...
public:
  ...
  [[nodiscard]] io_uring &get_handle() noexcept { return ring_; }
